...
impl UserTableHandler for DatabaseManager {
    ...
    async fn insert_user(
        ...
        conn.execute(
            format!(
                "INSERT OR IGNORE INTO {} (user_address, block_number, health_factor) VALUES (?, ?, ?)",
                table_name.as_str()
            )
            .as_str(),
            (user_address, block_number as i64, health_factor),
        )
        .await?;
    ...
    async fn update_user_health_factor(
        ...
        if past_table_name != table_name {
            conn.execute(
                format!("INSERT OR IGNORE INTO {} (user_address, block_number, health_factor) VALUES (?, ?, ?)", table_name.as_str()).as_str(),
                params![user_address, block_number as i64, health_factor],
            )
            .await?;
            ...
            conn.execute(
                format!(
                    "UPDATE {} SET health_factor = ? WHERE user_address = ?",
                    table_name.as_str()
                )
                .as_str(),
                (health_factor, user_address),
            )
            .await?;
